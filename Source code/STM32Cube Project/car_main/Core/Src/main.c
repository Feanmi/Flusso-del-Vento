...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define MAX_TELEM_STATE 5
#define TELEM_WAIT_MS 1000 / MAX_TELEM_STATE
/* USER CODE END PD */
...
//  init_bluetooth();
  ...
  while (1)
  {
	  perform_wifi_control_step();

	  detect_malfunction();

	  perform_motor_control_step();
	  perform_servo_control_step();

	  if(HAL_GetTick() - telem_last_tick > TELEM_WAIT_MS){
		  switch (telem_state) {
			case ACCELEROMETER:
				read_accelleration();
				break;
